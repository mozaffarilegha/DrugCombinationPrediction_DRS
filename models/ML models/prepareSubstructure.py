...
# Drug features 
rowdrug = np.load('drugSmile_drugSubEmbed_2644.npy', allow_pickle= True)
drugDF = rowdrug.item()
concatenated_data = {key: np.concatenate(drugDF[key]) for key in drugDF}
drug = pd.DataFrame.from_dict(concatenated_data, orient='index')
...
for k in range(nfolds):
    ...
    concatenated_df.to_csv(f"{features_folder}{k}_train_substructure.csv", index=False)
    ...
    concatenated_df_test.to_csv(f"{features_folder}{k}_test_substructure.csv", index=False)
    ...
    concatenated_df_val.to_csv(f"{features_folder}{k}_val_substructure.csv", index=False)
