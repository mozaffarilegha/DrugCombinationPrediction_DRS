...
def process_fold(train_data, y_train, test_data, y_test, y_test_binary, fold_index):
    ###
    xgb_model = xgb.XGBRegressor(n_estimators=200, max_depth=9, min_child_weight=1, learning_rate=0.4, gamma=0.1)
    ...
    result_df.to_csv(f"Result/XGB_DEG_fold_{fold_index}_metrics.csv", index=False)

    # Save y_pred to a CSV file inside the models folder
    pd.DataFrame(y_pred, columns=["Predicted"]).to_csv(f"Result/XGB_DEG_fold_{fold_index}_y_pred.csv", index=False)
