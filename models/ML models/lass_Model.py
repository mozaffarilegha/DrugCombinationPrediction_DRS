...
def process_fold(train_data, y_train, test_data, y_test, y_test_binary, fold_index):
    ...
    result_df.to_csv(f"Result/Lasso_DEG_SEN_RES_fold_{fold_index}_metrics.csv", index=False)

    # Save y_pred to a CSV file inside the models folder
    pd.DataFrame(y_pred, columns=["Predicted"]).to_csv(f"Result/Lasso_DEG_SEN_RES_fold_{fold_index}_y_pred.csv", index=False)
...
#Parallel(n_jobs=5)(delayed(process_fold_in_parallel)(0) for _ in range(1))
