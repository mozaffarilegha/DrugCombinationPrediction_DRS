...
def process_fold(train_data, y_train, test_data, y_test, y_test_binary, fold_index):
    # Define the AdaBoost classifier with the best hyperparameters
    adaboost_reg = AdaBoostRegressor(n_estimators=100, learning_rate=0.01)
    adaboost_reg.fit(train_data, y_train)

    y_pred = adaboost_reg.predict(test_data)
    ...
    result_df.to_csv(f"Result/AdaBoost_DEG_SEN_RES_fold_{fold_index}_metrics.csv", index=False)

    # Save y_pred to a CSV file inside the models folder
    pd.DataFrame(y_pred, columns=["Predicted"]).to_csv(f"Result/AdaBoost_DEG_SEN_RES_fold_{fold_index}_y_pred.csv", index=False)
...
def process_fold_in_parallel(k):
    ...
    train = pd.read_csv(f"{features_folder}{k}_train_DEG_SEN_RES.csv")
    test = pd.read_csv(f"{features_folder}{k}_test_DEG_SEN_RES.csv")
...
#Parallel(n_jobs=20)(delayed(process_fold_in_parallel)(0) for _ in range(1))
